...
@section('title', 'Registrasi Pemilik - RSHP UNAIR')
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Registrasi Pemilik</li>
@endsection
...
@section('content')
    ...
    <div class="info-banner mb-4">
        ...
        <div class="info-banner-content">
            <h6>Tentang Data Pemilik</h6>
            <p>Kelola data pemilik hewan yang terdaftar di RSHP UNAIR. Pastikan data kontak dan alamat selalu diperbarui</p>
        </div>
    </div>
        ...
        <div class="card-header">
            <div class="card-header-left">
                ...
                <div class="header-text">
                    <h5>Daftar Pemilik Hewan</h5>
                    <p>Total: <span class="badge badge-count">{{ $pemilik->count() }}</span> pemilik terdaftar</p>
                </div>
            </div>
        </div>
        ...
        <div class="card-body p-0">
            @if($pemilik->count() > 0)
                ...
                <div class="table-controls">
                    <div class="controls-left">
                        ...
                        <div class="search-box">
                            ...
                            <input type="text" id="searchInput" placeholder="Cari nama pemilik atau email">
                        </div>
                    </div>
                    
                    <!-- Tombol Tambah Pemilik di kanan -->
                    <div class="controls-right">
                        <a href="{{ route('resepsionis.registrasi.pemilik.create') }}" class="btn-add">
                            <i class="bi bi-plus-circle-fill"></i>
                            <span class="btn-text">Registrasi Pemilik Baru</span>
                        </a>
                    </div>
                </div>
                                ...
                                <td>
                                    <div class="table-main-info">
                                        ...
                                        <div class="user-info">
                                            ...
                                            <small>ID: PML{{ str_pad($item->idpemilik, 4, '0', STR_PAD_LEFT) }}</small>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <div class="email-info">
                                        <span class="email-text">{{ $item->user->email }}</span>
                                    </div>
                                </td>
                                <td>
                                    <div class="contact-info">
                                        <span class="wa-number">{{ $item->no_wa }}</span>
                                    </div>
                                </td>
                                <td>
                                    <div class="address-info">
                                        <span class="address-text">{{ Str::limit($item->alamat, 50) }}</span>
                                    </div>
                                </td>
                                <td>
                                    <div class="action-buttons">
                                        <a href="{{ route('resepsionis.registrasi.pemilik.edit', $item->idpemilik) }}" 
                                        ...
                                        <button data-id="{{ $item->idpemilik }}" 
                                                data-name="{{ $item->user->nama }}"
                ...
                <div class="table-footer">
                    <div class="showing-entries">
                        ...
                        <span id="showingText">Menampilkan 1 hingga {{ min(10, $pemilik->count()) }} dari {{ $pemilik->count() }} data</span>
                    </div>
            ...
            @else
                <div class="empty-state">
                    ...
                    <p>Silakan tambahkan data pemilik terlebih dahulu untuk memulai</p>
                    <a href="{{ route('resepsionis.registrasi.pemilik.create') }}" class="btn-empty">
                        <i class="bi bi-plus-circle-fill"></i>
                        <span>Tambah Pemilik Pertama</span>
                    </a>
                </div>
            @endif
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body text-center">
                    ...
                    <p class="modal-text">Apakah Anda yakin ingin menghapus data pemilik <strong id="deleteItemName"></strong>?</p>
    ...
    <style>
        ...
        .email-info, .contact-info, .address-info {
            display: flex;
            flex-direction: column;
        }

        .email-text, .wa-number, .address-text {
            font-size: 0.9rem;
            color: var(--text-dark);
            font-weight: 500;
        }
...
@section('extra-js')
    <script>
        ...
        function searchTable() {
            ...
            // Di dalam DOMContentLoaded, setelah allRows diisi:
allRows = Array.from(document.querySelectorAll('#tableBody tr'));


             // TAMBAHKAN SORTING DI SINI
    filteredRows.sort((a, b) => {
        const namaA = a.cells[1].textContent.toLowerCase(); // Kolom nama
        const namaB = b.cells[1].textContent.toLowerCase(); // Kolom nama
        return namaA.localeCompare(namaB); // A-Z
    });
        ...
        function confirmDelete(id, name) {
            ...
            form.action = `/resepsionis/registrasi/pemilik/${id}`;
